...
public:
    static constexpr uint64_t magic_number = 0x5345415354524653; // SEASTRFS
    static constexpr uint32_t max_shards_nb = 500;
    static constexpr disk_offset_t min_alignment = 4096;
    ...
    // TODO: follow the all-or-nothing public member principle
    bootstrap_record() = default;
    bootstrap_record(uint64_t version, disk_offset_t alignment, disk_offset_t cluster_size, inode_t root_directory,
...
struct bootstrap_record_disk {
    ...
    uint32_t crc;
}; // FIXME: make packed and have defined endianness (e.g. like ELF file does) + replace version with feature bitmap
