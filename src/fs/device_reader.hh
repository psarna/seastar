...
class device_reader {
    ...
    struct disk_data_raw {
        ...
        uintptr_t data_alignment(disk_offset_t alignment) {
            return mod_by_power_of_2(reinterpret_cast<uintptr_t>(_data), alignment);
        }

        size_t range_alignment(disk_offset_t alignment) {
...
public:
    device_reader(block_device& device, disk_offset_t alignment, const io_priority_class& pc = default_priority_class())
        : _device(device)
