...
future<size_t> device_reader::read(uint8_t* buffer, disk_offset_t disk_offset, size_t read_len) {
    device_reader::disk_data_raw disk_data_to_read = {
        ._disk_range = {
            .beg = disk_offset,
            .end = disk_offset + read_len},
        ._data = buffer,
    };
    if (size_t cache_read_len = 0;
            !_cached_disk_read._data.empty() &&
...
future<size_t> device_reader::do_read(device_reader::disk_data_raw disk_data_to_read) {
    ...
    if (remaining_disk_data->data_alignment(_alignment) == remaining_disk_data->range_alignment(_alignment)) {
        disk_range read_range = {
            .beg = round_down_to_multiple_of_power_of_2(remaining_disk_data->_disk_range.beg, _alignment),
            .end = round_up_to_multiple_of_power_of_2(remaining_disk_data->_disk_range.beg, _alignment),
        };
        // Read prefix
