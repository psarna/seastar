...
class unlink_or_remove_file_operation {
    ...
    future<> unlink_or_remove(std::string path, remove_semantics remove_semantics) {
        ...
        assert(path.empty() || path.back() == '/'); // Hence fast-check for "is directory" is done in path_lookup
        ...
        return _shard.path_lookup(path).then([this](inode_t dir_inode) {
            ...
            if (_entry_inode_info->is_directory()) {
                ...
                if (!_entry_inode_info->get_directory().entries.empty()) {
            ...
            if (_entry_inode_info->is_directory()) {
                return _shard._locks.with_locks(shard::locks::unique{
                    .inode = dir_inode,
                    .dentry = _entry_name
                }, shard::locks::unique{
                    .inode = _entry_inode,
    ...
    future<> unlink_or_remove_file_in_directory() {
        ...
        auto entry_it = _dir_info->entries.find(_entry_name);
        if (entry_it == _dir_info->entries.end() || entry_it->second != _entry_inode) {
            return make_exception_future(operation_became_invalid_exception());
        }
        ...
        if (!_entry_inode_info->is_linked() && !_entry_inode_info->is_open()) {
